...
```sh
DSPY_CACHEDIR=/private/tmp/dspy-toon-cache .venv/bin/python -m benchmarks.qwen_adapter_comparison run --concurrency 50
DSPY_CACHEDIR=/private/tmp/dspy-toon-cache .venv/bin/python -m benchmarks.qwen_adapter_comparison score \
  --sob benchmark_results/qwen_20260905/scorers/sob \
  --bbeh benchmark_results/qwen_20260905/scorers/bbeh \
  --tablebench benchmark_results/qwen_20260905/scorers/tablebench
```

The frozen requests can also be replayed independently from requests.jsonl. Pinned upstream scoring files and their available license files are preserved under scorers/; the score command above uses those snapshots. cases.jsonl contains the selected source records and golds; requests.jsonl contains only model-visible inputs and schemas, never reference answers.

## Concurrency amendment

After 88 completed main requests at concurrency four, the user explicitly requested 50 in parallel. That partial run and its original manifest are preserved in partial_concurrency4/ and excluded from the final comparison. Four in-flight requests were cancelled; their token usage is unknown. All 800 unchanged frozen requests restart at concurrency 50. This was an operational change, not performance-based case selection or prompt tuning. The partial run and preflight warm the server cache; latency remains descriptive.

## Replaying after the prompt fixes

The run command checks recorded source hashes as well as frozen requests. The original baseline source now differs from the working tree because the documented prompt fixes were applied; its source_snapshot/ preserves that implementation. The commands above describe the original phase, not permission to overwrite current code. For a new experiment use prepare with a fresh output directory and the current source. Scoring uses the currently imported parsers; the archived scorer modules preserve task-metric behavior. All reported response parsers were unchanged by the two prompt-only fixes.
